...
all_tests_test_() ->
    {inorder, {foreach,
               ...
               [
                ...
                fun dgol_evolve_at_next_time/0
               ]}}.
...
cell_can_evolve_at_next_time() ->
    Self = self(),
    CellPosition = {2, 2},
    {ok, _} = dgol:start_universe_and_wait(5, 5, [CellPosition], 50),
...
cells_can_evolve_together() ->
    ...
    {ok, _} = dgol:start_universe_and_wait(5, 5, StepOneLivingCells, 50),
...
events_about_cells_are_emitted_correctly() ->
    {ok, _} = dgol:start_universe_and_wait(5, 5, [{1,1}], 50),
...
cell_can_recover_after_death() ->
    {ok, _} = dgol:start_universe_and_wait(5, 5, [], 50),
    universe:evolve_at(5),
    ?assertEqual(5, universe:target_time()),
    timer:sleep(500),
